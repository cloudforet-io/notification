...
def NotificationInfo(notification_vo: Notification, minimal=False):
    ...
    if not minimal:
        ...
        if notification_vo.project_id:
            info.update({'project_id': notification_vo.project_id})

        if notification_vo.user_id:
            info.update({'user_id': notification_vo.user_id})

        if notification_vo.parent_notification_id:
            info.update({'user_id': notification_vo.parent_notification_id})
